...
export default function KidsMissionDashboard() {
  ...
  const [tempTargetItem, setTempTargetItem] = useState('');
  const [tempTargetPoints, setTempTargetPoints] = useState(10000);
  ...
  const fetchMissionsData = async () => {
    try {
      ...
      if (data.success) {
        ...
        setCurrentPoints(data.wallet.currentPoints);
        setTargetItem(data.wallet.targetItem);
        setTargetPoints(data.wallet.targetPoints);
        setTempTargetItem(data.wallet.targetItem);
        setTempTargetPoints(data.wallet.targetPoints);
      }
    } catch (error) {
  ...
  const handleAddMission = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert('미션 배포 실패: ' + data.error);
      }
    } catch (err) {
      console.error(err);
      alert('서버 오류로 미션을 배포하지 못했습니다.');
    }
  };
  ...
  const handleApprove = async (regId: number) => {
    try {
      ...
      if (data.success) {
        fetchMissionsData();
        alert('미션이 승인되었습니다! 자녀의 지갑에 포인트가 지급되었습니다.');
      } else {
        alert('미션 승인 실패: ' + data.error);
      }
    } catch (err) {
      console.error(err);
  ...
  const handleRequest = async (id: number) => {
    ...
    try {
      ...
      if (data.success) {
        fetchMissionsData();
        alert('완료 요청을 보냈습니다! 부모님이 승인하면 포인트가 적립됩니다.');
      } else {
        alert('완료 요청 실패: ' + data.error);
      }
    } catch (err) {
      console.error(err);
      alert('서버 오류가 발생했습니다.');
    ...
    try {
      const res = await fetch('/api/missions', {
        ...
        body: JSON.stringify({
          ...
          targetPoints: Number(tempTargetPoints)
        })
      });
      const data = await res.json();
      if (data.success) {
  ...
  // Parent Action: Cash payout and deduct points
  const handleCashPayout = async () => {
    try {
      const res = await fetch('/api/missions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        fetchMissionsData();
        alert('토스 송금이 완료되었고 자녀의 포인트가 정상 차감되었습니다!');
      } else {
        alert('환전 승인 실패: ' + data.error);
      }
    } catch (err) {
      console.error(err);
      alert('서버 오류가 발생했습니다.');
  ...
  const progressPercent = targetPoints > 0 ? Math.min(100, Math.round((currentPoints / targetPoints) * 100)) : 0;
        ...
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <nav className="hidden md:flex space-x-6 text-sm font-medium text-slate-600 items-center">
              <a href="/" className="hover:text-amber-600 transition-colors">장기요양계산기</a>
              <span className="font-semibold text-amber-600">자녀효도퀘스트</span>
              <a href="/os" className="hover:text-amber-600 transition-colors">가족돌봄OS</a>
                    ...
                    <input 
                      ...
                      onBlur={async () => {
                        await fetch('/api/missions', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ action: 'UPDATE_TARGET', targetItem, targetPoints })
                        });
                      }}
                    ...
                    {missions.filter(m => m.status === 'REQUESTED').map(mission => (
                      <div key={mission.id} className="bg-white rounded-2xl shadow-md shadow-rose-100 border border-rose-100 p-5 flex flex-col sm:flex-row justify-between gap-4">
                        ...
                        <div className="flex items-center">
                          <button 
                            onClick={() => handleApprove(mission.regId!)}
                ...
                {currentPoints >= targetPoints && (
                  <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-2xl shadow-lg p-6 text-white flex flex-col sm:flex-row justify-between items-center gap-4 animate-in slide-in-from-bottom-4">
                    <div>
                      ...
                      <p className="text-amber-100 text-sm">자녀가 목표({targetItem})를 달성하여 {targetPoints.toLocaleString()}P 환전 지급이 가능합니다.</p>
                    </div>
                    <button 
            ...
            {role === 'CHILD' && (
              <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="bg-gradient-to-br from-amber-400 to-amber-500 rounded-3xl p-8 text-amber-950 shadow-xl shadow-amber-200/50 relative overflow-hidden">
                  <SparklesIcon className="absolute top-4 right-4 w-24 h-24 text-amber-300 opacity-50 pointer-events-none" />
                          ...
                          <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <div className="flex items-center space-x-2">
                              <input 
                                ...
                                onChange={e => setTempTargetPoints(Number(e.target.value) || 0)}
                          ...
                          {!isExchangeRequested ? (
                            <button 
                              onClick={() => {
                                ...
                                alert('부모님께 용돈 환전 지급 알림이 전송되었습니다! 부모님께서 송금해 주실 때까지 기다려 주세요.');
                              }} 
